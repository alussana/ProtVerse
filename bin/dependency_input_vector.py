...
def main():
    ...
    dependency_table_file = 'input/dependency_table.tsv'
    """
    gene_pairs_file = sys.argv[1]
    dependency_table_file = sys.argv[2]

    gene_pairs = pd.read_csv(gene_pairs_file, sep='\t', header=None)
    gene_pairs.columns = ['label', 'reactome', 'gene_1', 'gene_2']

    dependency = pd.read_csv(dependency_table_file, sep='\t', index_col=0, header=None)
    ...
    for i in range(len(gene_pairs)):
        ...
        gene_1_expr = dependency.loc[dependency.index == gene_1, ].transpose()
        gene_2_expr = dependency.loc[dependency.index == gene_2, ].transpose()
    ...
    gene_pairs['DepMap Linfoot MI'] = norm_mutual_info
    gene_pairs['DepMap CCC'] = ccc_coef
    gene_pairs[r'DepMap $\cos^2\theta$'] = cos_sim
    gene_pairs[r'DepMap $\mathrm{sign}\,(\cos\theta)$'] = cos_sign
    gene_pairs[r'DepMap $r^2$'] = pearson
    gene_pairs[r'DepMap $\mathrm{sign}\,(r)$'] = pearson_sign
    gene_pairs[r'DepMap $r^2$ Sig.'] = pearson_pval
    gene_pairs[r'DepMap $r_s^2$'] = spearman
    gene_pairs[r'DepMap $\mathrm{sign}\,(r_s)$'] = spearman_sign
    gene_pairs[r'DepMap $r_s^2$ Sig.'] = spearman_pval
    gene_pairs['DepMap Max CR'] = max_codetection
    gene_pairs['DepMap Min CR'] = min_codetection
    gene_pairs['DepMap Tot CR'] = tot_codetection
    gene_pairs['DepMap Data'] = data_flag
    gene_pairs['DepMap Overlap'] = overlap_flag
