...
def main():
    ...
    ax.plot([0, 1], [0, 1], linestyle="--", lw=1, color="black", label="Chance", alpha=0.8)
    ax.set(
        xlim=[-0.05, 1.05],
        ylim=[-0.05, 1.05],
        title=f'Metabolic Interactions (Training)\nMCC = {mcc_train}; ACC = {acc_train}',
        xlabel='FPR',
        ylabel='TPR',
    )
    ...
    plt.axhline(y=0.5, color='black', linestyle='--', label='Chance', lw=1, alpha=0.8)
    ax.set_aspect(1 / ax.get_data_ratio())
    ax.legend(loc="lower right", frameon=False)
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    ...
    ax.set(
        xlim=[-0.05, 1.05],
        ylim=[-0.05, 1.05],
        title=f'Metabolic Interactions (Validation)',
        xlabel='Recall',
        ylabel='Precision'
    )
    ax.set_aspect(1 / ax.get_data_ratio())
