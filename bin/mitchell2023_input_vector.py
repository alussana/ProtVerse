...
def main():
    ...
    mitchell2023_table_file = 'input/mitchell2023_table.tsv'
    """
    gene_pairs_file = sys.argv[1]
    mitchell2023_table_file = sys.argv[2]

    gene_pairs = pd.read_csv(gene_pairs_file, sep='\t', header=None)
    gene_pairs.columns = ['label', 'reactome', 'gene_1', 'gene_2']

    mitchell2023 = pd.read_csv(mitchell2023_table_file, sep='\t', index_col=0, header=None)
    ...
    for i in range(len(gene_pairs)):
        ...
        gene_1_expr = mitchell2023.loc[mitchell2023.index == gene_1, ].transpose()
        gene_2_expr = mitchell2023.loc[mitchell2023.index == gene_2, ].transpose()
        ...
        spearman.append(s)
        spearman_sign.append(s_sign)
        spearman_pval.append(s_pval)
        cos_sim.append(cs)
        cos_sign.append(cs_sign)
        ccc_coef.append(c)
        norm_mutual_info.append(nmi)
        max_codetection.append(max_cr)
        min_codetection.append(min_cr)
        tot_codetection.append(tot_cr)
        data_flag.append(data_available)
        overlap_flag.append(overlap_available)


    gene_pairs['PRIDE (perturb.) Linfoot MI'] = norm_mutual_info
    gene_pairs['PRIDE (perturb.) CCC'] = ccc_coef
    gene_pairs[r'PRIDE (perturb.) $\cos^2\theta$'] = cos_sim
    gene_pairs[r'PRIDE (perturb.) $\mathrm{sign}\,(\cos\theta)$'] = cos_sign
    gene_pairs[r'PRIDE (perturb.) $r^2$'] = pearson
    gene_pairs[r'PRIDE (perturb.) $\mathrm{sign}\,(r)$'] = pearson_sign
    gene_pairs[r'PRIDE (perturb.) $r^2$ Sig.'] = pearson_pval
    gene_pairs[r'PRIDE (perturb.) $r_s^2$'] = spearman
    gene_pairs[r'PRIDE (perturb.) $\mathrm{sign}\,(r_s)$'] = spearman_sign
    gene_pairs[r'PRIDE (perturb.) $r_s^2$ Sig.'] = spearman_pval
    gene_pairs['PRIDE (perturb.) Max CR'] = max_codetection
    gene_pairs['PRIDE (perturb.) Min CR'] = min_codetection
    gene_pairs['PRIDE (perturb.) Tot CR'] = tot_codetection
    gene_pairs['PRIDE (perturb.) Data'] = data_flag
    gene_pairs['PRIDE (perturb.) Overlap'] = overlap_flag
