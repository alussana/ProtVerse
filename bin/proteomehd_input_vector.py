...
def main():
    ...
    protomehd_table_file = 'input/protomehd_table.tsv'
    """
    gene_pairs_file = sys.argv[1]
    protomehd_table_file = sys.argv[2]

    gene_pairs = pd.read_csv(gene_pairs_file, sep='\t', header=None)
    gene_pairs.columns = ['label', 'reactome', 'gene_1', 'gene_2']

    protomehd = pd.read_csv(protomehd_table_file, sep='\t', index_col=0, header=None)
    ...
    for i in range(len(gene_pairs)):
        ...
        gene_1_expr = protomehd.loc[protomehd.index == gene_1, ].transpose()
        gene_2_expr = protomehd.loc[protomehd.index == gene_2, ].transpose()
            ...
            if all(np.isnan(a)) or all(np.isnan(b)) or (a == 0).all() or (b == 0).all():
                ...
                if len(a_filtered) == 0 or len(b_filtered) == 0:
                    ...
                    if codetection > 3 and np.std(a_filtered) != 0 and np.std(b_filtered) != 0:
                        ...
                        #r2 = round(np.corrcoef(a_filtered, b_filtered)[0,1], 3)
                        # pearson correlation
    ...
    gene_pairs['ProteomeHD Linfoot MI'] = norm_mutual_info
    gene_pairs['ProteomeHD CCC'] = ccc_coef
    gene_pairs[r'ProteomeHD $\cos^2\theta$'] = cos_sim
    gene_pairs[r'ProteomeHD $\mathrm{sign}\,(\cos\theta)$'] = cos_sign
    gene_pairs[r'ProteomeHD $r^2$'] = pearson
    gene_pairs[r'ProteomeHD $\mathrm{sign}\,(r)$'] = pearson_sign
    gene_pairs[r'ProteomeHD $r^2$ Sig.'] = pearson_pval
    gene_pairs[r'ProteomeHD $r_s^2$'] = spearman
    gene_pairs[r'ProteomeHD $\mathrm{sign}\,(r_s)$'] = spearman_sign
    gene_pairs[r'ProteomeHD $r_s^2$ Sig.'] = spearman_pval
    gene_pairs['ProteomeHD Max CR'] = max_codetection
    gene_pairs['ProteomeHD Min CR'] = min_codetection
    gene_pairs['ProteomeHD Tot CR'] = tot_codetection
    gene_pairs['ProteomeHD Data'] = data_flag
    gene_pairs['ProteomeHD Overlap'] = overlap_flag
